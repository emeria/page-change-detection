...
// Load an image as a binary string
function loadImageAsBinaryString(imagePath) {
  try {
...
const crawlPage = async (page, url) => {
  ...
  // Get screenshot
 const screenshotPath = `screenshots/` + currentRunDate + `/${url.replace(/\W+/g, '-')}.png`;
 await page.screenshot({ path: screenshotPath, fullpage: true });
  

/**
 * Screenshot to text to hash
 */
let screenshotText = "";
screenshotText = loadImageAsBinaryString(screenshotPath);

  fs.appendFileSync(logOutputPath, 'Hashing for: ' + url+'\n');
  // Calculate the hash of the page content
  const hash = calculateHash(screenshotText);
  ...
  // fs.writeFile(contentFile, screenshotText, (err) => {
  //   if (err) {
    ...
    if (historicalRecord.hash === hash) { //if the hash is the same, no changes have been made
      ...
      fs.unlink(screenshotPath, (err) => {
        if (err) {
          ...
          console.log('File deleted successfully: ', screenshotPath);
        }
      });
      return;
    } else { 
